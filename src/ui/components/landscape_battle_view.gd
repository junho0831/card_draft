...
func setup(owner_battle, old_root: Control, action_panel: Control) -> void:
	...
	unit_width = floorf((viewport.x - 120 - 24 - 68 - 24) / 5.0)
	...
	header.add_child(action("메뉴", func(): battle.main._show_main_menu(), 44))
	battle.battle_guidance_label.reparent(page)
	battle.battle_guidance_label.autowrap_mode = TextServer.AUTOWRAP_OFF
	battle.battle_guidance_label.clip_text = true
	battle.battle_guidance_label.text_overrun_behavior = TextServer.OVERRUN_TRIM_ELLIPSIS
	battle.battle_guidance_label.add_theme_font_size_override("font_size", 14)
	...
	lane(board, battle.opponent_field_box, true)
	battle.enemy_intent_label.reparent(board)
	battle.enemy_intent_label.add_theme_font_size_override("font_size", 14)
	battle.enemy_intent_label.custom_minimum_size = Vector2.ZERO
	battle.enemy_intent_label.autowrap_mode = TextServer.AUTOWRAP_OFF
	battle.enemy_intent_label.clip_text = true
	lane(board, battle.player_field_box, false)
	battle.hand_scroll.reparent(board)
	battle.hand_scroll.custom_minimum_size = Vector2(0, hand_size.y + 6)
	battle.hand_scroll.size_flags_vertical = Control.SIZE_EXPAND_FILL
...
func lane(parent: Control, cards: HBoxContainer, enemy: bool) -> void:
	...
	var hero := Button.new()
	hero.custom_minimum_size = Vector2(68, 72)
	hero.clip_text = true
	...
	cards.reparent(row)
	cards.custom_minimum_size = Vector2(0, 72)
...
func tile(card: Dictionary, bottom: String, width: float, accent: Color, height: float = 72) -> Button:
	var button := Button.new()
	...
	stamp(button, "CardName", String(card.get("name", "")), Vector2(3, height - 43), Vector2(width - 6, 20), Color(tint.r * 0.35, tint.g * 0.35, tint.b * 0.35, 0.96), 14)
	if card.has("attack"):
...
func field_slot(side: Dictionary, index: int, ally: bool) -> Control:
	if index >= side.field.size():
		var empty := tile({}, "+" if ally and index == side.field.size() else "", unit_width, Color(0.2, 0.27, 0.33))
	...
	var button := tile(unit, status, unit_width, accent)
	if ally and battle.selected_attacker == index:
	...
	if not prediction.is_empty():
		...
		stamp(button, "EnemyPrediction", "적%d→%d" % [unit.health, prediction.defender_health], Vector2(2, 23), Vector2(unit_width - 4, 23), Color(0.45, 0.08, 0.05, 0.95))
		stamp(button, "OwnTargetPrediction", "내%d→%d" % [battle._selected_player_attacker().health, prediction.attacker_health], Vector2(2, 46), Vector2(unit_width - 4, 23), Color(0.06, 0.23, 0.38, 0.95))
		if prediction.defender_health <= 0:
			stamp(button, "Lethal", "처치", Vector2(unit_width - 36, 2), Vector2(34, 19), Color(0.45, 0.08, 0.05), 12)
		button.tooltip_text = "공격 후 적 체력 / 내 체력 (장비·사망 효과 별도)"
	if ally and battle.selected_attacker == index and battle.opponent.field.size() == 1:
		var target: Dictionary = battle.opponent.field[0]
		var preview: Dictionary = battle._predict_unit_attack(unit, target, battle.player, battle.opponent)
		stamp(button, "OwnPrediction", "내%d→%d" % [unit.health, preview.attacker_health], Vector2(2, 23), Vector2(unit_width - 4, 23), Color(0.06, 0.23, 0.38, 0.95))
