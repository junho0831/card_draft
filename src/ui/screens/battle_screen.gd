...
var auto_end_pending := false
var floating_text_until := 0
...
var battle_action_caption_label: Label
...
func _recommended_action_state() -> Dictionary:
	...
	if selected_attacker != -1:
		...
		if selected.is_empty():
			return {
				"kind": "end_turn",
				"text": "턴 넘기기",
				"guidance": "더 할 행동이 없습니다. '여기 눌러 턴 넘기기' 버튼을 누르세요.",
			}
		...
		if not _enemy_vanguard_blocks_hero() and int(opponent.get("health", 0)) <= _predict_hero_attack_damage(selected, player, false):
			return {
				...
				"guidance": "'여기 눌러 승리하기' 버튼을 누르세요. %s가 적 영웅을 쓰러뜨립니다." % String(selected.get("name", "유닛")),
				"outcome": "victory",
			}
	...
	if ready_attacker_index != -1 and ready_attacker_index < player.field.size():
		...
		if not _enemy_vanguard_blocks_hero() and int(opponent.get("health", 0)) <= _predict_hero_attack_damage(ready_attacker, player, false):
			return {
				...
				"guidance": "'여기 눌러 승리하기' 버튼을 누르세요. 공격자와 대상은 자동으로 선택됩니다.",
	...
	return {
		"kind": "end_turn",
		"text": "턴 넘기기",
		"guidance": "이번 턴 행동이 끝났습니다. '여기 눌러 턴 넘기기' 버튼을 누르세요.",
	}
...
func _combo_status_text() -> String:
	...
	if tag.is_empty() or streak <= 0:
		if finisher_used:
			var finisher_tag := String(battle_state.get("combo_finisher_tag", ""))
			var finisher_meta: Dictionary = main._build_tag_meta().get(finisher_tag, {})
			return "%s 피니시 완료" % String(finisher_meta.get("name", finisher_tag))
...
func _compact_unit_hover_text(unit: Dictionary, card_def: Dictionary, is_player_field: bool) -> String:
	...
	if not equipment_names.is_empty():
		parts.append("장비: %s" % ", ".join(equipment_names))
	return "\n".join(parts)

func _make_battle_guidance_panel(compact: bool) -> PanelContainer:
	var panel = _make_battle_surface(Color(0.035, 0.045, 0.065, 0.5), Color(0.22, 0.3, 0.39, 0.4), 0, 8, 2)
	var tight = _is_tight_battle_layout()
	var wide_tight = _is_wide_tight_battle_layout()
	var mobile = _is_mobile_battle_layout()
	panel.custom_minimum_size = Vector2(0, 30 if mobile else (24 if wide_tight else (34 if tight else (44 if compact else 46))))
	...
	panel.add_child(row)
	var title: Label = main._make_label("직접 해보기" if main.Onboarding.first_battle(main.current_run) else "다음 행동", 11 if mobile else (12 if tight else (14 if compact else 16)), Color(0.48, 0.72, 1.0, 1.0))
	title.horizontal_alignment = HORIZONTAL_ALIGNMENT_LEFT
	title.autowrap_mode = TextServer.AUTOWRAP_OFF
	title.custom_minimum_size = Vector2(0 if mobile else (80 if tight else (96 if compact else 125)), 0)
	title.visible = false
	row.add_child(title)
	ally_cancel_button = null # Cancel is always available in the bottom action bar.
	...
	battle_objective_label.visible = false
...
func _make_battle_tutorial_panel(compact: bool) -> PanelContainer:
	...
	panel.visible = false # Full tutorial text lives in the detail overlay.
...
func _uses_touch_hand_selection() -> bool:
	return bool(main.touch_input_active)
...
func _battle_field_card_size() -> Vector2:
	...
	if _is_tight_battle_layout():
		return Vector2(132, 154)
	return Vector2(156, 182) if not _is_compact_layout() else Vector2(132, 154)

func _battle_hand_card_size() -> Vector2:
	if _is_mobile_battle_layout():
		return PRESENTATION.hand_size(true, false, false)
	if _is_wide_tight_battle_layout():
		return PRESENTATION.hand_size(false, true, _is_roomy_wide_battle_layout())
	if _is_tight_battle_layout():
		return Vector2(140, 178)
	return Vector2(190, 220) if not _is_compact_layout() else Vector2(162, 194)
...
func _add_field_lane(parent: VBoxContainer, lane: HBoxContainer, lane_height: int, mobile: bool) -> void:
	...
	lane_scroll.mouse_filter = Control.MOUSE_FILTER_PASS
...
func _play_combo_finisher_feedback(result: Dictionary) -> void:
	var tag := String(result.get("tag", ""))
	var headline := String(result.get("headline", "연계 피니시"))
	var detail := String(result.get("detail", ""))
	var meta: Dictionary = main._build_tag_meta().get(tag, {})
	var color: Color = meta.get("color", Color(1.0, 0.82, 0.28, 1.0)).lightened(0.18)
	var target := _hero_target_for_player(true)
	if String(result.get("target", "player")) == "enemy":
		target = _hero_target_for_player(false)
	elif String(result.get("target", "player")) == "field" and not player.field.is_empty():
		target = _field_slot_for(player, 0)
	_add_log("3연계 피니시: %s · %s" % [headline, detail])
	_record_build_trigger(tag, "%s!" % headline, target, color, true)
...
func _make_top_status_bar(compact: bool) -> PanelContainer:
	...
	turn_timer_label.custom_minimum_size = Vector2(46 if mobile else (118 if tight else (132 if compact else 170)), 0)
...
func _make_battle_hero_art(hero_art: int, size: Vector2, enemy_side: bool) -> TextureRect:
	if enemy_side:
		...
		if String(enemy.get("id", "")) == "goblin_raider":
			return main._make_card_art_rect({"art_id": "goblin_raider"}, size)
...
func _make_hero_target(side: Dictionary, hero_art: int, enemy_target: bool, compact: bool) -> Control:
	if _is_wide_tight_battle_layout():
		return _make_reference_hero(side, hero_art, enemy_target)
	var tight = _is_tight_battle_layout()
	var wide_tight = _is_wide_tight_battle_layout()
	var mobile = _is_mobile_battle_layout()
	...
	if enemy_target:
		...
		button.text = ""
		button.focus_mode = Control.FOCUS_NONE
		button.custom_minimum_size = Vector2(0, 42 if mobile else (38 if wide_tight else (46 if tight else (72 if compact else 64))))
		...
		panel.custom_minimum_size = Vector2(0, 40 if mobile else (32 if wide_tight else (40 if tight else (62 if compact else 54))))
		panel.size_flags_horizontal = Control.SIZE_EXPAND_FILL
		node = panel
	...
	var art: TextureRect = _make_battle_hero_art(hero_art, Vector2(28, 28) if mobile else (Vector2(24, 24) if wide_tight else (Vector2(30, 30) if tight else (Vector2(50, 50) if compact else Vector2(42, 42)))), enemy_target)
...
func _make_deck_preview_panel(compact: bool, min_height: int) -> Dictionary:
	...
	deck_list_label.fit_content = true
	...
	deck_list_label.add_theme_font_size_override("normal_font_size", 14)
...
func _make_battle_action_panel(compact: bool) -> PanelContainer:
	...
	var action_accent := Color(1.0, 0.74, 0.24, 1.0) if guidance_mode != GUIDANCE_MODE_HINT else Color(0.42, 0.68, 1.0, 1.0)
	var panel := PanelContainer.new()
	panel.add_theme_stylebox_override('panel', BATTLE_STYLES.make_action_dock_style(action_accent, 8 if wide_tight else 12))
	panel.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	var landscape_phone: bool = mobile
	...
	battle_action_caption_label.custom_minimum_size = Vector2(0 if not wide_tight else 168, 0)
	battle_action_caption_label.size_flags_horizontal = Control.SIZE_EXPAND_FILL if not wide_tight else Control.SIZE_SHRINK_CENTER
	...
	var primary_height := 56 if mobile else 56 if wide_tight else 64 if tight else 70 if compact else 78
	...
	if vertical_stack and not landscape_phone:
		recommended_action_button.size_flags_horizontal = Control.SIZE_SHRINK_CENTER
	_style_recommended_action_button(recommended_action_button, "play_card", "", guidance_mode)
	recommended_action_button.add_theme_font_size_override('font_size', 18 if mobile else 16 if tight else 20)
	...
	race_power_button.custom_minimum_size = Vector2(0, 56 if mobile else 44 if wide_tight else 46 if tight else 50 if compact else 54)
	...
	end_turn_button.custom_minimum_size = Vector2(0, 56 if mobile else 40 if wide_tight else 44 if tight else 46 if compact else 50)
...
func _make_detail_toggle_row(compact: bool) -> HBoxContainer:
	...
	detail_toggle_button.custom_minimum_size = Vector2(124 if tight else 144, 30 if tight else 34)
	_style_detail_action_button(detail_toggle_button, false)
	detail_toggle_button.add_theme_font_size_override('font_size', 10 if tight else 12)
	detail_toggle_button.pressed.connect(Callable(self, "_toggle_battle_details"))
...
func _make_battle_detail_panel(compact: bool) -> PanelContainer:
	...
	battle_objective_label.reparent(outer)
	battle_objective_label.visible = true
	battle_objective_label.horizontal_alignment = HORIZONTAL_ALIGNMENT_LEFT
	battle_objective_label.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART
	battle_objective_label.clip_text = false
	if _should_show_battle_tutorial():
		var help: Dictionary = _battle_tutorial_content()
		outer.add_child(main._make_label("\n".join(help.get("lines", [])), 14, Color(0.86, 0.9, 0.96)))
		var understood := Button.new()
		understood.text = "안내 확인"
		understood.custom_minimum_size.y = 44
		understood.pressed.connect(func():
			_dismiss_battle_tutorial()
			understood.disabled = true
		)
		outer.add_child(understood)
	...
	var bottom_row: BoxContainer = VBoxContainer.new()
...
func _toggle_battle_details() -> void:
	...
	if not battle_detail_visible: _check_no_actions_loss()
...
func _make_battle_surface(bg_color: Color, accent_color: Color, border_width: int = 1, radius: int = 8, margin: int = 10) -> PanelContainer:
	var panel := BATTLE_STYLES.make_battle_surface(bg_color, accent_color, border_width, radius, margin)
...
func _refresh_status_chips() -> void:
	for key in reference_health_bars:
		var bar: ProgressBar = reference_health_bars[key]
		var side: Dictionary = opponent if key == "enemy" else player
...
func _build_battle_ui() -> void:
	...
	battle_action_caption_label = null
	...
	log_label = null
	hand_scroll = null
	var field_card_size := _battle_field_card_size()
	var hand_card_size := _battle_hand_card_size()
	var field_height := int(field_card_size.y + (12 if mobile else 4))
	var hand_height := int(hand_card_size.y + (18 if mobile else (26 if touch_hand else (18 if wide_tight else 42))))
	...
	var top_action_panel = _make_battle_action_panel(compact)
	...
	center_column.mouse_filter = Control.MOUSE_FILTER_PASS
	center_column.add_theme_constant_override("separation", 5 if tight else 6)
	battle_root.add_child(center_column)

	var board_panel = _make_battle_surface(Color(0.025, 0.034, 0.045, 0.38), Color(0.16, 0.28, 0.38, 0.5), 1, 12, 6 if tight else 8)
	board_panel.size_flags_horizontal = Control.SIZE_EXPAND_FILL
	board_panel.size_flags_vertical = Control.SIZE_SHRINK_BEGIN
	PRESENTATION.open_surface(board_panel, 2)
	...
	board_box.mouse_filter = Control.MOUSE_FILTER_PASS
	...
	opponent_field_box.add_theme_constant_override("separation", 8 if tight else 10)
	if wide_tight:
		enemy_row.add_child(opponent_field_box)
	else:
	...
	player_field_box.add_theme_constant_override("separation", 8 if tight else 10)
	if wide_tight:
		var player_row := HBoxContainer.new()
		player_row.add_theme_constant_override("separation", 20)
		board_box.add_child(player_row)
		player_row.add_child(_make_hero_target(player, PLAYER_HERO_ART, false, compact))
		player_row.add_child(player_field_box)
	else:
	...
	PRESENTATION.open_surface(hand_panel, 2)
	bottom_row.add_child(hand_panel)
	var hand_box_wrap: BoxContainer = VBoxContainer.new()
	hand_box_wrap.add_theme_constant_override("separation", 4 if tight else 6)
	hand_box_wrap.mouse_filter = Control.MOUSE_FILTER_PASS
	hand_panel.add_child(hand_box_wrap)
	var hand_header: BoxContainer = VBoxContainer.new() if wide_tight else HBoxContainer.new()
	if wide_tight:
		hand_header.visible = false
		hand_header.custom_minimum_size.x = 0
		hand_header.alignment = BoxContainer.ALIGNMENT_CENTER
	hand_header.add_theme_constant_override("separation", 8)
	hand_header.mouse_filter = Control.MOUSE_FILTER_PASS
	hand_box_wrap.add_child(hand_header)
	var hand_title: Label = main._make_label("내 손패", 14, Color(0.92, 0.95, 1.0, 1.0))
	...
	hand_box.size_flags_horizontal = Control.SIZE_SHRINK_BEGIN
	...
	hand_scroll.resized.connect(Callable(self, "_layout_hand_cards"))
...
func _work_on_hand_card_pressed(index: int, target_unit_id: int = -1, confirmed: bool = false) -> void:
	...
	if target_unit_id < 0 and not confirmed and not _is_landscape_phone() and _uses_touch_hand_selection() and selected_hand_slot != hand_slot:
		selected_hand_slot = hand_slot
		_render_hand()
		hand_render_signature = _hand_signature()
		_play_sfx("hover")
	...
	var had_vanguard := _enemy_vanguard_blocks_hero()
	main.battle_effects.play_card(player, opponent, card, _battle_effect_context("player", target_unit_id))
	if had_vanguard and not _enemy_vanguard_blocks_hero():
	...
	if card_type == "unit" and player.field.size() > old_field_size:
		...
		_record_first_play_action("summoned")
...
func _apply_equipment_attack_triggers(attacker: Dictionary, attacker_side: Dictionary, defender_side: Dictionary) -> void:
	...
	if hero_damage <= 0 and draw_amount <= 0 and heal_amount <= 0:
		return
	_play_sfx("combo")
	if hero_damage > 0:
		_play_effect_hit_feedback(_hero_target_for_player(defender_side == player), "잿불 %d" % hero_damage, Color(1.0, 0.42, 0.2, 1.0))
	if draw_amount > 0:
		_play_effect_hit_feedback(_hero_target_for_player(attacker_side == player), "드로우 +%d" % draw_amount, Color(0.42, 0.76, 1.0, 1.0))
	if heal_amount > 0:
		_play_effect_hit_feedback(_hero_target_for_player(attacker_side == player), "흡혈 +%d" % heal_amount, Color(0.42, 1.0, 0.62, 1.0))
...
func _check_no_actions_loss() -> void:
	if auto_end_pending or not _can_auto_end_turn(): return
	auto_end_pending = true
	call_deferred("_finish_exhausted_turn", battle_state, int(battle_state.get("player_turn_count", 0)))

func _can_auto_end_turn() -> bool:
	if leaving_battle or main.Onboarding.first_battle(main.current_run): return false
...
func _finish_exhausted_turn(state: Dictionary, turn: int) -> void:
	while not leaving_battle and (inflight_actions > 0 or Time.get_ticks_msec() < floating_text_until):
		await main.get_tree().process_frame
	if is_same(state, battle_state) and int(battle_state.get("player_turn_count", 0)) == turn and _can_auto_end_turn():
		if is_instance_valid(landscape_view):
			landscape_view.center_guidance.text = "행동 완료 · 상대 턴"
		await main.get_tree().create_timer(0.6).timeout
		if is_same(state, battle_state) and int(battle_state.get("player_turn_count", 0)) == turn and _can_auto_end_turn():
			_add_log("더 이상 할 수 있는 행동이 없어 턴을 종료합니다.")
			await _on_end_turn_pressed()
	auto_end_pending = false
...
func _show_slot_overlay_text(target: Control, text: String, color: Color) -> void:
	if target == null or not is_instance_valid(target) or text.is_empty():
		return
	if is_instance_valid(landscape_view):
		_spawn_floating_text(target, text, color, 16)
		return
	var lbl := Label.new()
	lbl.text = text
...
func _show_card_board_preview(card: Dictionary, playable: bool) -> void:
	_clear_card_board_preview()
	if not pending_action.is_empty():
		return # Target-selection borders and its single badge already identify the action.
	...
	if card_type == "equipment":
		if player.field.is_empty():
			return
		var equipment_text := "공격 +2"
		match card_id:
			"ember_blade":
				equipment_text = "공격 +1 · 추가 피해"
			"wind_quiver":
				equipment_text = "공격 +1 · 드로우"
			"bone_armor":
				equipment_text = "체력 +3 · 죽음 피해"
			"royal_standard":
				equipment_text = "+1/+1"
				for i in range(player.field.size()):
					_add_card_preview_marker(_field_slot_for(player, i), equipment_text, accent.lightened(0.2))
				return
			"blood_blade":
				equipment_text = "공격 +2 · 영웅 피해 2"
			"war_horn":
				equipment_text = "공격 +1"
				if player.field.size() < player_field_slots.size():
					_add_card_preview_marker(player_field_slots[player.field.size()], "즉시 공격 1/1", Color(0.34, 0.82, 1.0, 1.0))
		_add_card_preview_marker(_field_slot_for(player, 0), equipment_text, accent.lightened(0.2))
		return
	var damage := _direct_damage_preview(card)
	if damage > 0:
...
func _resolve_ai_hero_attack(index: int) -> void:
	...
	_apply_equipment_attack_triggers(unit, opponent, player)
...
func _spawn_floating_text(target: Control, text: String, color: Color, font_size: int = 42, duration: float = 0.85, center_offset: Vector2 = Vector2.ZERO) -> void:
	...
	if is_instance_valid(landscape_view) and is_instance_valid(battle_fx_layer):
		_spawn_compact_floating_text(target, text, color, duration, center_offset)
		return
	var lbl = Label.new()
	lbl.text = text
	lbl.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
...
func _spawn_compact_floating_text(target: Control, text: String, color: Color, duration: float, offset: Vector2) -> void:
	var layer: Control = battle_fx_layer
	var bounds: Rect2 = landscape_view.board_scroll.get_global_rect().grow(-4)
	var numeric := text.trim_prefix("-").trim_prefix("+").is_valid_int()
	var lbl := Label.new()
	lbl.text = text
	lbl.add_theme_font_size_override("font_size", 22 if numeric else 16)
	lbl.add_theme_color_override("font_color", color)
	lbl.add_theme_color_override("font_outline_color", Color.BLACK)
	...
	lbl.size = Vector2(minf(200, bounds.size.x), 52)
	var at := target.global_position + target.size / 2 - lbl.size / 2 + offset
	at.x = clampf(at.x, bounds.position.x, bounds.end.x - lbl.size.x)
	at.y = clampf(at.y, bounds.position.y + 12, bounds.end.y - lbl.size.y)
	var now := Time.get_ticks_msec()
	var delay := maxf(0, (floating_text_until - now) / 1000.0)
	var lifetime := minf(duration, 0.5)
	floating_text_until = now + int((delay + lifetime) * 1000)
	layer.add_child(lbl)
	lbl.global_position = at
	lbl.modulate.a = 0
	var tween := lbl.create_tween()
	tween.tween_interval(delay)
	tween.tween_property(lbl, "modulate:a", 1.0, 0.05)
	tween.tween_property(lbl, "position:y", lbl.position.y - 12, lifetime - 0.05)
	tween.parallel().tween_property(lbl, "modulate:a", 0.0, lifetime - 0.05)
...
func _spawn_center_banner(text: String, color: Color, font_size: int = 42, duration: float = 0.9) -> void:
	if text.is_empty() or main.modal_layer == null or not is_instance_valid(main.modal_layer):
		return
	if is_instance_valid(landscape_view) and not game_over and not battle_finished:
		_spawn_floating_text(player_hero_target, text, color, 16, duration)
		return
	var lbl := Label.new()
	lbl.text = text
	lbl.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
...
func _trigger_hype_moment(target: Control, text: String, color: Color, sfx_name: String, shake: float = 8.0, font_size: int = 42, center_banner: bool = false) -> void:
	...
	if center_banner and not is_instance_valid(landscape_view):
...
func _make_empty_field_slot(compact: bool, is_next_summon_slot: bool = false, is_player_field: bool = true) -> Control:
	...
	var emblem: Label = main._make_label(emblem_text, 26 if mobile else 24 if tight else 26 if compact else 30, emblem_color)
	...
	if is_next_summon_slot:
		var text: Label = main._make_label('다음 소환', 10 if tight else 10 if compact else 11, Color(0.64, 0.76, 0.9, 0.74))
		text.autowrap_mode = TextServer.AUTOWRAP_OFF
		text.mouse_filter = Control.MOUSE_FILTER_IGNORE
		box.add_child(text)
	elif enemy_clickable:
		var text: Label = main._make_label('영웅 공격' if selected_attacker >= 0 else '아군 먼저 선택', 10 if tight else 10 if compact else 11, Color(1.0, 0.58, 0.5, 0.78))
		text.autowrap_mode = TextServer.AUTOWRAP_OFF
...
func _build_field_slot(side: Dictionary, index: int, is_player_field: bool) -> Control:
	...
	var content_size = Vector2(visual_size.x - 12.0, visual_size.y - 10.0)
	var art_size = Vector2(content_size.x, content_size.y - (20 if wide_tight else (24 if tight else 30)))

	# Keep the lane geometry stable while the occupied card visually claims more space.
	var slot_root := Control.new()
	slot_root.custom_minimum_size = frame_size
	slot_root.mouse_filter = Control.MOUSE_FILTER_PASS
	slot_root.z_index = 1
	var frame = Button.new()
	frame.custom_minimum_size = visual_size
	frame.size = visual_size
	frame.position = (frame_size - visual_size) * 0.5
	...
	slot.add_child(main.ui.make_card_name_band(main, unit, 'field', compact, tight))
...
func _render_hand() -> void:
	...
	for i in range(player.hand.size()):
		...
		var is_recommended = i == recommended_index and (not main.Onboarding.first_battle(main.current_run) or _recommended_action_state().get("kind") == "play_card")
		...
		card_box.add_child(main.ui.make_card_header(main, card, "hand", compact, tight, cost))
		if not tight:
			card_box.add_child(main.ui.make_card_identity_label(main, card, "hand", compact, tight, true, false))
		var art_size := Vector2(content_size.x, 96 if mobile or wide_tight else 88)
		var hand_art: TextureRect = main.ui.make_card_art(main, card, art_size)
		hand_art.stretch_mode = TextureRect.STRETCH_KEEP_ASPECT_COVERED
		hand_art.size_flags_horizontal = Control.SIZE_EXPAND_FILL
		var art_wrap := Control.new()
		art_wrap.custom_minimum_size = art_size
		art_wrap.mouse_filter = Control.MOUSE_FILTER_IGNORE
		card_box.add_child(art_wrap)
		hand_art.set_anchors_and_offsets_preset(Control.PRESET_FULL_RECT)
		art_wrap.add_child(hand_art)
		var preview_text = _card_result_preview(card)
		var rules_text := String(card.get("text", "")).strip_edges()
		var visible_rules := "" if tight else rules_text
		...
		if String(card.get("type", "")) == "unit":
			var attack_badge: PanelContainer = main.ui.make_stat_badge(str(int(card.get("attack", 0))), Color(0.46, 0.16, 0.12), true)
			attack_badge.mouse_filter = Control.MOUSE_FILTER_IGNORE
			attack_badge.position = Vector2(3, art_size.y - 34)
			art_wrap.add_child(attack_badge)
			var health_badge: PanelContainer = main.ui.make_stat_badge(str(int(card.get("health", 0))), Color(0.1, 0.24, 0.46), true)
			health_badge.mouse_filter = Control.MOUSE_FILTER_IGNORE
			health_badge.position = Vector2(art_size.x - 33, art_size.y - 34)
			art_wrap.add_child(health_badge)
		...
		action_panel.visible = not mobile or not playable or (main.Onboarding.first_battle(main.current_run) and is_touch_selected)
		...
		var action_label: Label = main._make_label(action_text, 12 if mobile else 10 if tight else 11 if not compact else 9, Color(0.86, 1.0, 0.92, 1.0) if playable else Color(0.58, 0.62, 0.68, 1.0))
...
func _hand_signature() -> String:
	...
	if main.Onboarding.first_battle(main.current_run):
		parts.append("lesson_rec:%s" % _recommended_action_state().get("kind", ""))
...
func _refresh_status_labels() -> void:
	if is_instance_valid(hand_scroll):
		hand_scroll.modulate = Color(0.48, 0.48, 0.52) if not pending_action.is_empty() else Color.WHITE
	...
	if battle_focus_panel != null and is_instance_valid(battle_focus_panel):
		battle_focus_panel.visible = _should_show_battle_focus() and not main.Onboarding.first_battle(main.current_run)
...
func _refresh_action_buttons() -> void:
	...
	if detail_toggle_button != null and is_instance_valid(detail_toggle_button):
		if bool(detail_toggle_button.get_meta("header_toggle", false)):
			...
			detail_toggle_button.text = "상세 닫기" if battle_detail_visible else "상세 정보"
	if battle_action_caption_label != null and is_instance_valid(battle_action_caption_label):
		battle_action_caption_label.text = _battle_action_caption_text(recommended_state)
		var caption_color := Color(1.0, 0.88, 0.52, 1.0)
		if recommended_kind == "wait":
			caption_color = Color(0.68, 0.74, 0.82, 1.0)
		elif _battle_guidance_mode() == GUIDANCE_MODE_HINT:
			caption_color = Color(0.72, 0.86, 1.0, 1.0)
		elif recommended_kind == "end_turn":
			caption_color = Color(0.72, 0.9, 1.0, 1.0)
		battle_action_caption_label.add_theme_color_override("font_color", caption_color)
	if race_power_button != null and is_instance_valid(race_power_button):
		var race_meta: Dictionary = main._current_race_meta()
		var race_color: Color = race_meta.get("color", Color(0.42, 0.68, 1.0, 1.0))
...
func _make_reference_hero(side: Dictionary, hero_art: int, enemy_target: bool) -> Control:
	var hero := Button.new()
	hero.custom_minimum_size = Vector2(248, 136)
	hero.focus_mode = Control.FOCUS_NONE
	var accent := Color(0.88, 0.22, 0.29) if enemy_target else Color(0.18, 0.63, 0.92)
	var surface := _make_modern_style(Color(0.02, 0.028, 0.045, 0.32), Color(accent, 0.25), 0, 6, 0)
	hero.add_theme_stylebox_override("normal", surface)
	hero.add_theme_stylebox_override("hover", surface)
	hero.add_theme_stylebox_override("pressed", surface)
	if enemy_target:
		hero.pressed.connect(Callable(self, "_attack_opponent_hero"))
		hero_attack_button = hero
		opponent_hero_target = hero
	else:
		player_hero_target = hero
	var row := HBoxContainer.new()
	row.set_anchors_and_offsets_preset(Control.PRESET_FULL_RECT)
	row.add_theme_constant_override("separation", 10)
	hero.add_child(row)
	var art := _make_battle_hero_art(hero_art, Vector2(88, 136), enemy_target)
	...
	row.add_child(info)
	var name_label: Label = main._make_label(String(side.get("name", "")), 16, Color(0.97, 0.95, 0.89))
	name_label.horizontal_alignment = HORIZONTAL_ALIGNMENT_LEFT
	info.add_child(name_label)
	var hp: Label = main._make_label("HP %d / %d" % [side.health, side.max_health], 15, accent.lightened(0.4))
	hp.horizontal_alignment = HORIZONTAL_ALIGNMENT_LEFT
	info.add_child(hp)
	var bar := ProgressBar.new()
	bar.show_percentage = false
	bar.custom_minimum_size = Vector2(0, 10)
	bar.max_value = int(side.max_health)
	bar.value = int(side.health)
	bar.add_theme_stylebox_override("background", _make_modern_style(Color(0.02, 0.025, 0.04), Color(0.15, 0.19, 0.25), 1, 4, 0))
	var fill := _make_modern_style(accent, accent.lightened(0.2), 1, 4, 0)
	fill.shadow_color = Color(accent, 0.3)
	fill.shadow_size = 5
	bar.add_theme_stylebox_override("fill", fill)
	info.add_child(bar)
	reference_health_bars["enemy" if enemy_target else "player"] = bar
	if enemy_target:
		opponent_hero_target_hp_label = hp
		var badge := _make_battle_badge(_hero_attack_target_badge_text(), Color(0.08, 0.035, 0.04, 0.7), accent, 11)
		info.add_child(badge)
		opponent_hero_target_badge = badge
		opponent_hero_target_badge_label = badge.get_meta("text_label") as Label
	else:
		player_hero_target_hp_label = hp
		reference_mana_label = main._make_label("", 13, Color(0.3, 0.76, 1.0))
		reference_mana_label.horizontal_alignment = HORIZONTAL_ALIGNMENT_LEFT
		info.add_child(reference_mana_label)
	_ignore_field_decoration_input(row)
	return hero

# Optional run data: old saves derive their next prompt from the live board.
func _record_first_play_action(action: String) -> void:
	if not main.Onboarding.first_battle(main.current_run):
		return
	var actions: Dictionary = main.current_run.get("first_play_actions", {})
	actions[action] = true
	main.current_run["first_play_actions"] = actions
	...
	match String(state.get("kind", "wait")):
		"play_card":
			...
			if index >= 0 and index < player.hand.size():
				target = _hand_card_control(int(player.hand[index].get("_hand_slot", index)))
				if target != null and hand_scroll != null:
