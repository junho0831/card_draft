...
Spring Boot 서버 카드 seed 데이터:

```text
server/src/main/resources/cards.json
```

현재는 두 파일을 수동으로 맞춰야 한다. 카드 이름, 비용, 스탯, 설명을 수정하면 두 파일을 같이 수정한다.
...
서버는 `rarity`, `enabled`가 없으면 각각 `일반`, `true`로 처리한다. Godot 데이터에는 현재 이 두 필드가 없어도 된다.
...
ID는 효과 처리, 덱 저장, 보유 카드 수량, 서버 DB에서 모두 기준 키로 쓰인다. 한번 출시한 ID는 가급적 바꾸지 않는다.
...
- 유닛 소환 효과: `scripts/main.gd`의 `_play_card`
- 주문 효과: `scripts/main.gd`의 `_resolve_spell`
- 사망 효과: `scripts/main.gd`의 `_cleanup_side_dead`
- 장착 효과: `scripts/main.gd`의 `_play_card`
...
- `healing_spring`: 내 영웅 체력 3 회복.
- `elf_bow`: 가장 앞의 아군 유닛 공격력 +2.
- `grave_knight`: 사망 시 영웅 체력 2 회복.

새 효과 카드를 만들 때는 카드 JSON을 추가한 뒤 해당 함수에 카드 ID 분기를 추가한다.

## 밸런스 기준

상세 기준은 `res://docs/balance.md`를 따른다.
