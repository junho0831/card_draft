...
- 전투
  ...
  - 손패는 현재 `HFlowContainer` 기반이라 좁은 폭에서 줄바꿈 대응이 가능해야 한다.
  - 다만 최종 목표는 `Control` 기반 수동 부채꼴 손패다. 이 변경은 반응형 검증 이후 별도 적용한다.
- 보상
  - 선택 가능한 카드 3장이 중심이다.
...
- 공통 패널 자산: `assets/ui/fantasy_ui_panel.png`
...
- 드래그 중 타깃 프리뷰
- 모바일 탭 확대 카드
- hover 시 확대뿐 아니라 회전 복원과 y축 상승을 함께 적용
