...
자동 UI 확대는 최대 1.45배다. PC 모니터에서는 1920x1080에서도 UI가 작아지지 않도록 Canvas Items 스케일을 1배에 가깝게 유지한다. `UI 크기` 설정의 크게/작게 보정은 1280x720 이상 PC·웹에서만 적용한다.
...
- 전투 본문은 논리 폭 1440px로 제한하고 가로 행동 도크는 420px를 유지한다.
